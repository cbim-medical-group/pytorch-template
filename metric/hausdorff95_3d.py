...
def hausdorff95_3d(output, target, misc):
    with torch.no_grad():
        ...
        for k in range(pred.shape[0]):
            for j in range(pred.shape[-1]):
                ...
                if np.count_nonzero(pred[k]) > 0:  # need to handle blank prediction
                    n += 1
                    pred_contours = pred[k,:,:,j] & (~morphology.binary_erosion(pred[k,:,:,j]))
                    target_contours = target[k,:,:,j] & (~morphology.binary_erosion(target[k,:,:,j]))
