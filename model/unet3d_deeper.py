...
class Unet3dDeeper(nn.Module):
    def __init__(self, in_channel=3, out_channel=1, channel_start=64, padding=1):
        super().__init__()
        # down conv
        ...
        self.c5 = ConvDownBlock(channel_start * 8, channel_start * 16, padding=padding,kernel_size=(2,2,1))
        self.cu = ConvDownBlock(channel_start * 16, channel_start * 32, padding=padding,kernel_size=(2,2,1))
        # up conv
...
# def test():
#     net = Unet3dDeeper(in_channel=4, out_channel=4).cuda()
#     x = torch.randn(1,4,192,192,8).cuda()
#
#     y = net(x)
#     print(y.size())
#
