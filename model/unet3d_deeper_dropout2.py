...
class Unet3dDeeperDropout2(nn.Module):
    def __init__(self, in_channel=3, out_channel=1, channel_start=64, padding=1, dropout_rate=0.0):
        super().__init__()
        # down conv
        self.c1 = ConvDownBlock(in_channel, channel_start, padding=padding, dropout_rate=dropout_rate)
        self.c2 = ConvDownBlock(channel_start, channel_start * 2, padding=padding, dropout_rate=dropout_rate)
        self.c3 = ConvDownBlock(channel_start * 2, channel_start * 4, padding=padding, dropout_rate=dropout_rate)
        self.c4 = ConvDownBlock(channel_start * 4, channel_start * 8, padding=padding,kernel_size=(2,2,1), dropout_rate=dropout_rate)
        ...
        # up conv
        self.u4 = ConvUpBlock(channel_start * 32, channel_start * 16,kernel_size=(2,2,1), stride=(2,2,1), dropout_rate=dropout_rate)
        self.u5 = ConvUpBlock(channel_start * 16, channel_start * 8,kernel_size=(2,2,1), stride=(2,2,1), dropout_rate=dropout_rate)
