...
def split_train_test_dt(source_path, source_file, target_file_pattern="new_cap_process_#.h5", rate=0.2):
    ...
    for key in list(keys):
        ...
        print(f"save:{key}, la:{la_label.shape}, sa:{sa_label_dt.shape}")

        label = np.concatenate((sa_label_dt, la_label))
        data = np.concatenate((sa_data, la_data))

        h5_file.create_dataset(f"{type}/{key}/data", data=data, compression='gzip')
        h5_file.create_dataset(f"{type}/{key}/label", data=label, compression='gzip')
...
split_train_test_dt("/research/cbim/vast/qc58/pub-db/CAP/process/MC-Net", "processed_data.h5")
